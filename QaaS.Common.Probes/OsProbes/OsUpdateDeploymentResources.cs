    ...
    : BaseOsUpdateDeploymentWithGlobalDict<OsUpdateResourcesProbeConfig>
{
        ...
        IConfiguration localConfiguration)
    {
        ...
        if (!string.IsNullOrWhiteSpace(replicaSetName) && !string.IsNullOrWhiteSpace(containerName))
        {
            yield return new ProbeGlobalDictReadRequest("recovery",
                BuildGlobalDictionaryAliasPath("Os", "Recovery", "Resources", "Deployment", replicaSetName,
                    containerName));
        }
    }
    ...
    protected override IReadOnlyList<string> GetRecoveryAliasPath()
        => BuildGlobalDictionaryAliasPath("Os", "Recovery", "Resources", "Deployment", Configuration.ReplicaSetName!,
